...
public class MainActivity extends AppCompatActivity {
    ...
    // ── Views ─────────────────────────────────────────────────────────────────
    private SwitchCompat swEnabled, swRootBypass, swPhoneSpoof, swShowMsg;
    ...
    private void setupListeners() {
        ...
        // Auto-detect carrier on number change
        etNumber.addTextChangedListener(new TextWatcher() {
            @Override public void beforeTextChanged(CharSequence s, int st, int c, int a) {}
            @Override public void onTextChanged(CharSequence s, int st, int b, int c) {}
            @Override public void afterTextChanged(Editable s) {
                applyCarrier(s.toString(), true);
            }
        });

        // Random generators
        findViewById(R.id.btn_rand_number).setOnClickListener(v -> {
            etNumber.setText(randPhone()); });
        findViewById(R.id.btn_rand_imei).setOnClickListener(v -> {
            etImei.setText(randImei()); });
        findViewById(R.id.btn_rand_aid).setOnClickListener(v -> {
            etAndroidId.setText(randHex(16)); });
    ...
    // ── Carrier auto-detect ────────────────────────────────────────────────
    private void applyCarrier(String raw, boolean showToast) {
        ...
        if (showToast)
            Toast.makeText(this, "📡 " + info.operator + " detected!", Toast.LENGTH_SHORT).show();
    }
    ...
    private void doSave() {
        ...
        Toast.makeText(this,
            "✅ Config saved! Target app force-stop karke reopen karo.",
    ...
    private String genImsi(String mccMnc) {
        ...
        int need = 15 - pfx.length();
        StringBuilder sb = new StringBuilder(pfx);
        for (int i = 0; i < need; i++) sb.append(rng.nextInt(10));
        return sb.toString();
    }
    ...
    private String randPhone() {
        int[] pfx = {7, 8, 9};
        return "+91" + pfx[rng.nextInt(3)] + String.format("%09d", rng.nextInt(1_000_000_000));
    }
